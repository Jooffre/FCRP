...
public class DeferredShadows
{
    ...
    void RenderMainLightCascadeShadowmap(ref ScriptableRenderContext context, ref CullingResults cullingResults)
    {
        ...
        //buffer.SetGlobalVectorArray(PID.shadowCascadeBiasID, m_cascadeBias);
    ...
    void SetupMainLightShadowReceiverConstants(VisibleLight shadowLight, bool supportsSoftShadows)
    {
        ...
        buffer.SetGlobalInt(PID.cascadeCountID, k_MaxCascades);
        ...
        buffer.SetGlobalTexture(PID.mainLightShadowmapID, m_MainLightShadowmapTexture);

        buffer.SetGlobalMatrixArray(PID.worldToShadowMatID, m_MainLightShadowMatrices);

        buffer.SetGlobalVector(PID.shadowParamsID, new Vector4(light.shadowStrength, softShadowsProp, shadowFadeScale, shadowFadeBias));
        buffer.SetGlobalVector(PID.cascadeZDistanceID, 
        ...
        buffer.SetGlobalVector(PID.shadowFadingDataID, new Vector4(1f / 50f, 1f / 0.1f, 1f / (1f - f * f), 0));

        if (m_ShadowCasterCascadesCount > 1)
        {
            buffer.SetGlobalVectorArray(PID.cascadeShadowSplitSpheresID, m_CascadeSplitDistances);

            buffer.SetGlobalVector
            (
                PID.cascadeShadowSplitSphereRadiiID,
        ...
        // {
        //     buffer.SetGlobalVector(PID.shadowOffset0ID, new Vector4(-invHalfShadowAtlasWidth, -invHalfShadowAtlasHeight, 0.0f, 0.0f));
        //     buffer.SetGlobalVector(PID.shadowOffset1ID, new Vector4(invHalfShadowAtlasWidth, -invHalfShadowAtlasHeight, 0.0f, 0.0f));
        //     buffer.SetGlobalVector(PID.shadowOffset2ID, new Vector4(-invHalfShadowAtlasWidth, invHalfShadowAtlasHeight, 0.0f, 0.0f));
        //     buffer.SetGlobalVector(PID.shadowOffset3ID, new Vector4(invHalfShadowAtlasWidth, invHalfShadowAtlasHeight, 0.0f, 0.0f));
        ...
        //     buffer.SetGlobalVector(PID.shadowmapSizeID, new Vector4(invShadowAtlasWidth, invShadowAtlasHeight, renderTargetWidth, renderTargetHeight));
        // }
    }
